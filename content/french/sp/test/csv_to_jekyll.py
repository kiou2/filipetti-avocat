...
my_file = os.path.join(THIS_FOLDER, 'prout.csv')

# Open our data file in read-mode.
csvfile = open(my_file, 'r')
...
for row_index, row in enumerate(datareader):
	...
	# Othrwise, create a YAML file from the data in this row...
		...
		filename = os.path.join(THIS_FOLDER, row[0].lower().replace(" county", "").replace(" ", "_").replace("é", "e").replace("è", "e").replace("ê", "e").replace("à", "a") + '.md')
